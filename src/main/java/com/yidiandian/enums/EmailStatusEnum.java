...
public enum EmailStatusEnum {
    ...
    //根据code 找到desc 描述
    private static final Map<Integer, EmailStatusEnum> valueLookup = new ConcurrentHashMap<>(values().length);
    static {
        for (EmailStatusEnum type: EnumSet.allOf( EmailStatusEnum.class)){
    ...
    public static List<Map> typeEnumList() {
        ...
        for (EmailStatusEnum enumClass : EmailStatusEnum.values()) {
    ...
    public static EmailStatusEnum fromValue(Integer code) {
        EmailStatusEnum data = valueLookup.get(code);
        if (data == null) {
            throw new IllegalArgumentException("参数[" + code + "]不正确，没有找到对应的 Enum");
        }
        return data;
    }
    EmailStatusEnum(Integer code, String msg) {
        this.code = code;
        this.msg = msg;
    }
